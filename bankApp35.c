		...
		switch(choice) {
			...
			case 3:
				if(client1.balance <= MIN_BAL) {
					printf("\nYour current balance (%.2f) is the minimum required balance. Please deposit more to be able to withdraw.", client1.balance);
					clearScreen();
					break;
				}
...
void withdrawAmount(client *client1) {
	...
	do {
		...
		if(scanf("%f", &amount) != 1 || amount <= 0 || client1->balance - amount < MIN_BAL) {
			fflush(stdin);
			printf("\nError: Your withdrawal of %.2f will bring your balance below the minimum required balance (%.2f).", amount, MIN_BAL);
			clearScreen();
		}
