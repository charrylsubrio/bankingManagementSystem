        ...
        switch (choice) {
            ...
            case 3:
            	if(loggedClient->balance <= MIN_BAL) {
					printf("\n\t\t\t\t\t\t\tError: Your current balance (%.2lf) is the minimum required balance.\n", loggedClient->balance);
					clearScreen();
					break;
				}
...
//    scanf("%s", newAdmin.password);
    ...
    do {
        ...
        // Check password validity
        if (strlen(newAdmin.password) < 8 || !validatePassword(newAdmin.password)) {
            printf("\n\t\t\t\t\t\t\tPassword must be at least 8 characters long with at least 1 uppercase letter.\n");
            inputCtr = 1;
			clearScreen();
//        } else if (!validatePassword(newClient.password)) {
...
void loginAdmin() {
    ...
    int maxAttempts = 3; // Maximum number of attempts for password
    int attemptCount = 0; // Reset attempt count for this ID
    ...
    do {
        ...
        idFound = 0;  // Reset ID found flag for every new ID attempt
        for (int i = 0; i < adminCount; i++) {
            if (admins[i].id == id) {
                idFound = 1; // ID exists
                attemptCount = 0; // Reset attempt count for valid ID
        ...
        // If no matching ID is found, prompt the user again for the ID
        if (!idFound) {
            printf("\n\t\t\t\t\t\t\tInvalid ID. Please try again.\n");
            printf("\n\t\t\t\t\t\t\t===================================\n");
            clearScreen();
        }
    } while (!idFound); // Continue asking for a valid ID until found
}
...
void modifyClientDetails() {
    int id;
    char choice;
    int found = 0;  // Flag to track if the client was found
    ...
    do {
        ...
        found = 0;  // Reset found flag for each new ID input
        for (int i = 0; i < clientCount; i++) {
            if (clients[i].id == id) {
                found = 1;  // Set flag to true when client is found
                
                // Show current details and prompt for changes
                printf("\t\t\t\t\t\t\tNAME: %s\n", clients[i].name);
                // Loop for valid Yes/No response for name update
                do {
                    printf("\t\t\t\t\t\t\tUpdate Name? (Y/N): ");
                    scanf(" %c", &choice);  // Space before %c to clear the newline character
                } while (choice != 'Y' && choice != 'y' && choice != 'N' && choice != 'n');
                
                if (choice == 'y' || choice == 'Y') {
                    printf("\n\t\t\t\t\t\t\tNew Name: ");
                    scanf(" %[^\n]", clients[i].name);  // Read name with spaces
                }

                printf("\n\n\t\t\t\t\t\t\tCONTACT: %s\n", clients[i].contact);
                // Loop for valid Yes/No response for contact update
                do {
                ...
                if (choice == 'y' || choice == 'Y') {
                    printf("\n\t\t\t\t\t\t\tNew Contact: ");
                    scanf("%s", clients[i].contact);  // Read contact
                }

                printf("\n\n\t\t\t\t\t\t\tPASSWORD: [HIDDEN]\n");
                // Loop for valid Yes/No response for password update
                do {
                ...
                if (choice == 'y' || choice == 'Y') {
                    printf("\n\t\t\t\t\t\t\tNew Password: ");
                    scanf("%s", clients[i].password);  // Read password
                }
        ...
        // If client ID was not found, inform the user and prompt for a new ID
        if (!found) {
            printf("\n\n\t\t\t\t\t\t\tClient with ID %d not found. Please try again.\n", id);
            printf("\n\t\t\t\t\t\t\t===================================\n");
            clearScreen();
        }
    } while (!found);  // Repeat the loop until a valid client ID is found
}

void removeClient() {
    int id, i;
    int found = 0;  // Flag to track if the client is found
    ...
    do {
        ...
        for (i = 0; i < clientCount; i++) {
            if (clients[i].id == id) {
                found = 1;  // Client found
                
                // Remove the client by shifting others to fill the gap
        ...
        // If the client was not found, prompt for a new ID
        if (!found) {
            printf("\n\n\t\t\t\t\t\t\tClient with ID %d not found. Please try again.\n", id);
            printf("\n\t\t\t\t\t\t\t===================================\n");
            clearScreen();
        }
    } while (!found);  // Repeat until a valid ID is entered and the client is found
}

void searchClient() {
    int id;
    int found = 0;  // Flag to check if the client is found
    ...
    do {
        ...
        // Check if the user wants to exit the search process
        if (id == 0) {
            printf("\n\t\t\t\t\t\t\tExiting client search...\n");
            return;  // Exit the search and return to the previous menu
        }

        // Search for the client with the given ID
        found = 0; // Reset the flag for each new attempt
        for (int i = 0; i < clientCount; i++) {
            if (clients[i].id == id) {
                found = 1; // Client found
                printf("\n\t\t\t\t\t\t\tClient found!\n\n");
        ...
        // If the client ID is not found, display a message and retry
        if (!found) {
            printf("\n\n\t\t\t\t\t\t\tClient with ID %d not found. Please try again.\n", id);
            printf("\n\t\t\t\t\t\t\t===================================\n");
            clearScreen();
        }
    } while (!found);  // Continue asking for a valid ID until one is found
}
...
void viewTransacHist() {
    ...
    do {
        ...
        found = 0;  // Reset found flag for each new attempt

        // Iterate over the clients to find the one with the specified clientId
        for (int i = 0; i < clientCount; i++) {
            if (clients[i].id == clientId) {
                ...
                printf("\t\t\t\t\t\t\tCLIENT ID: %d\n", clients[i].id);
                printf("\t\t\t\t\t\t\tFULL NAME: %s\n\n", clients[i].name);
                found = 1;  // Mark that the client was found
                break;  // Exit the loop once the client is found
            }
        }

        // If no client found with the given ID, prompt the user again
        if (!found) {
            printf("\n\n\t\t\t\t\t\t\tNo client found with ID %d. Please try again.\n", clientId);
            printf("\n\t\t\t\t\t\t\t===================================\n");
            clearScreen();
            continue;  // Ask for the client ID again
        }

        // Display transaction history for the found client
        printf("\t\t\t\t\t\t-------------------------------------------------------\n");
        printf("\t\t\t\t\t\tDATE\t\tTIME\t\tTYPE\t\tBALANCE\n");
        printf("\t\t\t\t\t\t-------------------------------------------------------\n");

        // Iterate over the transactions to find those belonging to the specified client ID
        for (int i = 0; i < transactionCount; i++) {
        ...
        if (!found) {
            printf("\n\n\t\t\t\t\t\t\tNo transactions found for Client ID %d.\n", clientId);
        }

        printf("\n\t\t\t\t\t\t\t===================================\n");
        clearScreen();

    } while (!found);  // Repeat until transactions are found for the client
}
...
void displayAdminCred(Admin *loggedAdmin) {
    ...
    printf("\t\t\t\t\t\t\tADMIN NAME : %s", loggedAdmin->name);
