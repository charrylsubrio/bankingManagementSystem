        ...
        for (int i = 0; i < clientCount; i++) {
            if (clients[i].id == id) {
                ...
                if (strcmp(clients[i].password, password) == 0) {
                    ...
                    break; // Stay in the loop to ask for client ID and password again
                }
            }
        }
...
void loginAdmin() {
    ...
    while (1) { // Loop until valid credentials are entered
        idFound = 0; // Reset flag for every attempt
        ...
        for (int i = 0; i < adminCount; i++) {
            if (admins[i].id == id) {
                ...
                if (strcmp(admins[i].password, password) == 0) {
                    printf("Login successful. Welcome, %s!\n", admins[i].name);
                    displayAdminAction(); // Proceed to admin actions
        ...
        if (!idFound) {
            printf("Invalid Admin ID. Please try again.\n");
        }
    }
}
