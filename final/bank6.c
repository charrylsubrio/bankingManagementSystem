...
void registerClient() {
    ...
    do {
        int lengthValid = 1, uppercaseValid = 1;
        ...
        for (int i = 0; newClient.password[i] != '\0'; i++) {
            if (isupper(newClient.password[i])) {
                hasUpper = 1;
                break;
            }
        }
        ...
        // Continue asking for password if either condition is not met
    } while (strlen(newClient.password) < 8 || !isupper);
