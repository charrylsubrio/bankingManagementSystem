        ...
        switch (choice) {
            ...
            case 2:
            	if(loggedClient->balance <= MIN_BAL) {
					printf("\nError: Your current balance (%.2f) is the minimum required balance.", loggedClient->balance);
					break;
				}
				withdrawAmount(clients);
				break;
...
void withdrawAmount(Client *loggedClient) {
    ...
    while (1) {
        ...
        // Input validation and constraints check
        ...
        if (loggedClient->balance - amount < MIN_BAL) {
