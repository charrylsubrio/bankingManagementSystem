...
void acceptClient(client *client1) {
	...
	do {
		...
		printf("Enter Initial Balance (minimum %.2f): ", MIN_BAL);
		
		if(scanf("%f", &client1->balance) != 1 || client1->balance < MIN_BAL) {
			fflush(stdin);
			inputCtr = 1;
			printf("\nError: Initial balance must be at least %.2f.", MIN_BAL);
			clearScreen();
		}
